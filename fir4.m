...
f = (0:nfft-1)*(fs/nfft); % Frequency vector using the correct sampling frequency (fs)
originalFFT = abs(fft(audio, nfft)); % Magnitude of FFT

% Plot the original signal frequency spectrum
figure;
plot(f(1:floor(nfft/2)), originalFFT(1:floor(nfft/2)));
title('Original Signal Frequency Spectrum');
xlabel('Frequency (Hz)');
ylabel('Magnitude');

% Normalize the input signal
audio = audio / max(abs(audio)); % Normalize the signal to avoid clipping
...
% Design FIR bandpass filter with Hamming window for N=4
N = 4; % Filter order
b = fir1(N, normalized_cutoff, 'bandpass', hamming(N+1));

% Apply the filter to the signal
filtered_signal = filtfilt(b, 1, audio); % Zero-phase filtering

% Save the filtered audio file
audiowrite('filtered_audio_FIR_Hamming_N4_300to1500.wav', filtered_signal, fs);
...
frequencies = (0:length(filtered_signal)-1)*(fs/length(filtered_signal)); % Recalculate frequency vector
figure;
...
% Display a summary
disp('Filtered audio saved as: filtered_audio_FIR_Hamming_N4_300to1500.wav');
